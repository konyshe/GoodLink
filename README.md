...
| `--proxy` | Local端本地代理地址（可选） | `--proxy=0.0.0.0:1080` |
| `--forward` | Local端本地转发地址，多个用逗号间隔（可选） | `--forward=0.0.0.0:22@127.0.0.1:22,0.0.0.0:80@127.0.0.1:80` |
| `-v` | 查看版本信息（命令行版本） | `-v` |
...
### 本地转发模式（该模式下，TUN直连模式、TUN代理模式不会启动）
