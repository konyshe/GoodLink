...
10. 1024 以下是操作系统的保留端口, 基本都被占用了, local 端请使用 1024 以上端口。例如 linux 系统可以使用命令 `netstat -anp|grep 22` 判断 22 端口是否已被占用

11. 该项目刚刚起步, 功能还不完善, 欢迎到 Issues 上提出问题和建议, 帮忙测试的同学将保证永久免费使用

12. UI版本是从1.4才开发（goodlink-windows-amd64-ui.zip），但1.4还不稳定，未提交Docker(停留在1.3), 因此暂时不建议1.4搭配Docker使用，直到这一行文字被删除。如果无法使用, 请切换1.3正式版
