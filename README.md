...
**注: 该项目仅用于学习研究, 目前无商业合作，更无恶意行为。如果未来有广告之类盈利的行为，会郑重告知大家。**
...
3. 本程序即支持命令行方式, 也支持 docker 方式, windows 还有UI版本（cmd版本最稳定、性能最好、内存占用最小）, 可随意搭配
...
12. 连接remote端的windows远程桌面，可在ip后面加上 :13389，尝试连接。3389端口貌似有特殊路由处理，和goodlink虚拟网卡冲突

#### 💻 部署选项
| 平台 | 支持方式 | 推荐程度 | 说明 |
|------|----------|----------|------|
| Windows | 命令行 / UI界面 | ⭐⭐⭐⭐⭐ | 命令行版本最佳 |
| Linux | 命令行 / Docker | ⭐⭐⭐⭐⭐ | 支持 |
| macOS | 命令行 | ⭐⭐⭐⭐ | v2暂不支持 |
